...
def inner_grid(height, width):
    ...
    which the “inner” cells are all 1 and the cells on the outer border are all 0
    """
    grid = create_grid(height, width)   # initially all 0s
...
def random_grid(height, width):
    """ creates and returns a 2-D list of height rows and width columns in which the inner
    cells are randomly assigned either 0 or 1, but the cells on the outer border are all 0
    """
    grid = create_grid(height, width)   # initially all 0s
...
def copy(grid):
    ...
    height = len(grid)
    width = len(grid[0])
    new_grid = create_grid(height, width)   # initially all 0s
...
def invert(grid):
    """takes an existing 2-D list of 0s and 1s and inverts it – changing
    all 0 values to 1, and changing all 1 values to 0
    """
    height = len(grid)
    width = len(grid[0])
